...
class DockerSandbox:
    ...
    async def run(self, code: str, *, timeout_s: int) -> RunResult:
        ...
        # Dedicated artifacts dir mounted read-write for stable outputs
        artifacts_dir = Path(tempfile.mkdtemp(prefix="kts-art-"))
        try:
            ...
            # artifacts volume
            cmd.extend(["-v", f"{artifacts_dir}:/artifacts:rw"])
            # env caps
            for k, v in {**THREAD_CAPS, **self.env}.items():
                cmd.extend(["-e", f"{k}={v}"])
            cmd.append(self.image)
            # Run and export artifacts within the container
            entry = (
                f"{self.python_exe} main.py; "
                "if [ -f submission.csv ]; then cp submission.csv /artifacts/; fi"
            )
            cmd.extend(["sh", "-lc", entry])
            ...
            sub = artifacts_dir / "submission.csv"
            if sub.exists():
            ...
            # Do not remove artifacts_dir here; caller persists then cleans
